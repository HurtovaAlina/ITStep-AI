...
currency = input("Введіть валюту, яку ви хочете конвертувати ")
currency_exchange = input("Введіть валюту, в яку ви хочете конвертувати ")

# агент для визначення курсу валюти
agent1 = create_agent(
    model=llm,  # нейромережа агента
...
prompt1 = f"""
    ...
    Інформація, знайдена в інтернеті: використай інструмент google_search

    На основі цієї інформації визнач курс.
...
agent = create_agent(
    model=llm,  # нейромережа агента
    tools=[converter],  # список інструментів
)
...
6. Передай у converter номінал користувача та курс {exchange_data.exchange}.
...
# цикл зі спілкуванням
while True:
    # Запит від користувача
    user_query = input("Введіть номінал валюти, яку хочете конвертувати: ")
